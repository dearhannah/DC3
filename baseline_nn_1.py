...
def create_concatenated_input(data, X):
    """
    Create concatenated input from Q, p, A, G, h, X
    """
    batch_size = X.shape[0]
    
    # Flatten Q, p, A, G, h and repeat for each batch
    ...
    concatenated_input = torch.cat([Q_flat, p_flat, A_flat, G_flat, h_flat, X], dim=1)
    
    return concatenated_input

def get_concatenated_input_dim(data):
    """
    Calculate the dimension of concatenated input
    """
    Q_dim = data.Q.numel()
    p_dim = data.p.numel()
    A_dim = data.A.numel()
    ...
    return Q_dim + p_dim + A_dim + G_dim + h_dim + X_dim
...
def train_net(data, args, save_dir):
    ...
    for i in range(nepochs):
        ...
        for Xtrain in train_loader:
            ...
            # Extract original X from concatenated input for loss calculation
            X_original = extract_original_X(data, Xtrain)
            train_loss = softloss(data, X_original, Yhat_train, args)
...
def extract_original_X(data, concatenated_input):
    """
    Extract original X from concatenated input
    """
    Q_dim = data.Q.numel()
    p_dim = data.p.numel()
    A_dim = data.A.numel()
    ...
    # Calculate starting index for X in concatenated input
...
def eval_net(data, X, solver_net, args, prefix, stats):
    ...
    dict_agg(stats, make_prefix('loss'), softloss(data, X_original, Y, args).detach().cpu().numpy())
    ...
    dict_agg(stats, make_prefix('raw_ineq_max'), torch.max(data.ineq_dist(X_original, Y), dim=1)[0].detach().cpu().numpy())
    dict_agg(stats, make_prefix('raw_ineq_mean'), torch.mean(data.ineq_dist(X_original, Y), dim=1).detach().cpu().numpy())
    dict_agg(stats, make_prefix('raw_ineq_num_viol_0'),
             torch.sum(data.ineq_dist(X_original, Y) > eps_converge, dim=1).detach().cpu().numpy())
    dict_agg(stats, make_prefix('raw_ineq_num_viol_1'),
             torch.sum(data.ineq_dist(X_original, Y) > 10 * eps_converge, dim=1).detach().cpu().numpy())
    dict_agg(stats, make_prefix('raw_ineq_num_viol_2'),
             torch.sum(data.ineq_dist(X_original, Y) > 100 * eps_converge, dim=1).detach().cpu().numpy())
    dict_agg(stats, make_prefix('raw_eq_max'),
             torch.max(torch.abs(data.eq_resid(X_original, Y)), dim=1)[0].detach().cpu().numpy())
    dict_agg(stats, make_prefix('raw_eq_mean'),
             torch.mean(torch.abs(data.eq_resid(X_original, Y)), dim=1).detach().cpu().numpy())
    dict_agg(stats, make_prefix('raw_eq_num_viol_0'),
             torch.sum(torch.abs(data.eq_resid(X_original, Y)) > eps_converge, dim=1).detach().cpu().numpy())
    dict_agg(stats, make_prefix('raw_eq_num_viol_1'),
             torch.sum(torch.abs(data.eq_resid(X_original, Y)) > 10 * eps_converge, dim=1).detach().cpu().numpy())
    dict_agg(stats, make_prefix('raw_eq_num_viol_2'),
             torch.sum(torch.abs(data.eq_resid(X_original, Y)) > 100 * eps_converge, dim=1).detach().cpu().numpy())
...
class NNSolver(nn.Module):
    def __init__(self, data, args):
        ...
        # Calculate input dimension for concatenated input
