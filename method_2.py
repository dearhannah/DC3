    ...
    if args.get('useWandb', False) and WANDB_AVAILABLE:
        ...
        if run_name is None:
            ...
            if args.get('useSanity', False):
                run_name = f"sanity-{args['probType']}-{time_str}"
            else:
                run_name = f"regular-{args['probType']}-{time_str}"
...
def train_net(data, args, save_dir):
    ...
    for i in range(nepochs):
        ...
        # Create log message
        log_msg = 'Epoch {}: train loss {:.4f}, eval {:.4f}, dist {:.4f}, ineq max {:.4f}, ineq mean {:.4f}, ineq num viol {:.4f}, eq max {:.4f}, steps {}, time {:.4f}'.format(
...
def eval_net(data, X, solver_net, args, prefix, stats):
    ...
    dict_agg(stats, make_prefix('steps'), np.array([0]))  # No correction steps
    ...
    if hasattr(data, 'data_all'):
        ...
        dict_agg(stats, make_prefix('eval'), data.obj_fn(Ynew, X).detach().cpu().numpy())
    else:
        # This is a regular SimpleProblem dataset
        dict_agg(stats, make_prefix('eval'), data.obj_fn(Ynew).detach().cpu().numpy())
    dict_agg(stats, make_prefix('dist'), torch.norm(Ynew - Y, dim=1).detach().cpu().numpy())
    dict_agg(stats, make_prefix('ineq_max'), torch.max(data.ineq_dist(X, Ynew), dim=1)[0].detach().cpu().numpy())
    dict_agg(stats, make_prefix('ineq_mean'), torch.mean(data.ineq_dist(X, Ynew), dim=1).detach().cpu().numpy())
    ...
    dict_agg(stats, make_prefix('eq_mean'), torch.mean(torch.abs(data.eq_resid(X, Ynew)), dim=1).detach().cpu().numpy())
